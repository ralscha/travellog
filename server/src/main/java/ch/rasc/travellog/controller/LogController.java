  ...
  public SyncResponse<LogSync> sync(@RequestBody SyncRequest<LogSync> sync,
    ...
    if (sync.getUpdated() != null && !sync.getUpdated().isEmpty()) {
      ...
      for (LogSync clientLog : sync.getUpdated()) {
        var record = this.dsl
            .select(LOG.ID, LOG.UPDATED, LOG.TRAVEL_ID, LOG.CREATED, LOG.LAT, LOG.LNG,
                LOG.LOCATION, LOG.REPORT)
            .from(LOG)
            .where(LOG.ID.eq(clientLog.getId()).and(LOG.TRAVEL_ID.in(travelIds)))
            .fetchOne();

        if (record != null) {
          if (record.get(LOG.UPDATED).toEpochSecond(ZoneOffset.UTC) > clientLog.getTs()) {
            // db log is newer than the version sent from the client. ignore client
            // update
            get.add(new LogSync(record.get(LOG.ID),
                record.get(LOG.UPDATED).toEpochSecond(ZoneOffset.UTC),
                record.get(LOG.TRAVEL_ID),
                record.get(LOG.CREATED).toEpochSecond(ZoneOffset.UTC),
                record.get(LOG.LAT), record.get(LOG.LNG), record.get(LOG.LOCATION),
                record.get(LOG.REPORT)));
          }
          else {
            LocalDateTime now = LocalDateTime.now();
            int noOfUpdated = this.dsl.update(LOG)
                .set(LOG.CREATED,
                    LocalDateTime.ofInstant(Instant.ofEpochSecond(clientLog.getCreated()),
                        ZoneOffset.UTC))
                .set(LOG.LAT, clientLog.getLat()).set(LOG.LNG, clientLog.getLng())
                .set(LOG.LOCATION, clientLog.getLocation())
                .set(LOG.REPORT, clientLog.getReport()).set(LOG.UPDATED, now)
                .where(LOG.ID.eq(clientLog.getId()).and(LOG.TRAVEL_ID.in(travelIds)))
                .execute();
            if (noOfUpdated == 1) {
              updated.put(clientLog.getId(), now.toEpochSecond(ZoneOffset.UTC));
            }
          }
        }
      }
    ...
    if (sync.getInserted() != null && !sync.getInserted().isEmpty()) {
      ...
      for (LogSync clientLog : sync.getInserted()) {
        ...
        long id = this.dsl
            .insertInto(LOG, LOG.CREATED, LOG.LAT, LOG.LNG, LOG.LOCATION, LOG.REPORT,
                LOG.UPDATED, LOG.TRAVEL_ID)
            .values(
                LocalDateTime.ofInstant(Instant.ofEpochSecond(clientLog.getCreated()),
                    ZoneOffset.UTC),
                clientLog.getLat(), clientLog.getLng(), clientLog.getLocation(),
                clientLog.getReport(), now, clientLog.getTravelId())
            .returning(LOG.ID).fetchOne().getId();

        inserted.put(clientLog.getId(), new NewId(id, now.toEpochSecond(ZoneOffset.UTC)));
      }
    }
    ...
    if (sync.getGets() != null) {
      for (Long id : sync.getGets()) {
        var record = this.dsl
            .select(LOG.ID, LOG.UPDATED, LOG.TRAVEL_ID, LOG.CREATED, LOG.LAT, LOG.LNG,
                LOG.LOCATION, LOG.REPORT)
            .from(LOG).where(LOG.ID.eq(id).and(LOG.TRAVEL_ID.in(travelIds))).fetchOne();

        if (record != null) {
          get.add(new LogSync(record.get(LOG.ID),
              record.get(LOG.UPDATED).toEpochSecond(ZoneOffset.UTC),
              record.get(LOG.TRAVEL_ID),
              record.get(LOG.CREATED).toEpochSecond(ZoneOffset.UTC), record.get(LOG.LAT),
              record.get(LOG.LNG), record.get(LOG.LOCATION), record.get(LOG.REPORT)));
        }
      }
    }
